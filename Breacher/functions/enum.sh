...
#       create algo to check which version of snmp is runnign or pull it off a banner grab
        ...
        wait
        rm $IP/results/loot/raw/snmp_found
}
...
smb_enum (){
        ...
        if ! grep -q "smb-vuln-ms08-067:" "results/loot/smb/08-067";then rm $loot/smb/08-067;fi
        ...
        find ~ -path '*/$IP/results/loot/smb/*' -type f > $loot/smb/files
